...
# ~12%, 25% -- only 1000 tokens, k =100
# 4.77%, 5.35% -- 300_000 tokens, k = 250

MODEL_PATH = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", "models", "open_llama_3b"))
BATCH_SIZE = 100
NUM_TOKENS = 300_000  # Same as part1 5_000_000
LEARNING_RATE = 1e-5  
NUM_FEATURES = 50_000  # Same feature size as part1
NFM_K = 250  # NFM embedding dimension - modifiable parameter
...
def save_checkpoint(model, optimizer, scheduler, step, best_loss, metrics_history, checkpoint_dir):
    ...
    if metrics_history['val_best_loss'] == best_loss:
        best_model_path = checkpoint_dir / "best_nfm_model.pt"
...
class NeuralFactorizationMachine(nn.Module):
    def __init__(self, num_sae_features, embedding_dim, output_dim):
        ...
        # Linear component (first-order effects)
    ...
    def initialize_weights(self):
        ...
        nn.init.normal_(self.feature_embeddings.weight, std=0.0001)
    ...
    def forward(self, sae_features):
        ...
        # Get all embeddings and multiply by feature values
...
def train_nfm(nfm_model, train_loader, val_loader, num_steps, checkpoint_dir):
    ...
    try:
        ...
        for step in tqdm(range(num_steps), desc="Training NFM"):
            try:
...
def main():
    ...
    # Update NFM_K if provided via command line
    #global NFM_K
    #NFM_K = args.nfm_k
    ...
    # Initialize NFM
    print(f"Initializing Neural Factorization Machine with K={NFM_K}...")
    ...
    debug_model_parameters(nfm_model, "initial_nfm")
    ...
    final_model_path = "nfm_residual_model.pt"
    torch.save(nfm_model.state_dict(), final_model_path)
    print(f"Training complete! NFM saved as {final_model_path}")
    ...
    with torch.no_grad():
        ...
        residual_norm = torch.mean(sample_residual ** 2).item()
        nfm_prediction_norm = torch.mean(nfm_prediction ** 2).item()
        
        print(f"Original data MSE (baseline): {original_norm:.6f}")
        print(f"SAE1 only reconstruction error: {sae1_error:.6f}")
        print(f"SAE1 + NFM reconstruction error: {combined_error:.6f}")
